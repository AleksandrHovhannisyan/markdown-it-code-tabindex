...
// eslint-disable-next-line no-unused-vars
const { RenderRule } = require('markdown-it/lib/renderer');
// eslint-disable-next-line no-unused-vars
 ...
 * @param {any} env
 * @param {RenderRule} self
 */
const defaultRenderToken = (tokens, index, options, env, self) => self.renderToken(tokens, index, options);
...
const makeTabIndexRenderer = (existingRenderer, pluginOptions) => {
  ...
  return (tokens, index, renderOptions, env, self) => {
    ...
    switch (pluginOptions.target) {
      ...
      // <pre tabindex="0">
      case 'pre': {
        const defaultOutput = existingRenderer(tokens, index, renderOptions, env, self);
 ...
 * @param {MarkdownIt} md The markdown-it instance to customize.
...
const markdownItCodeTabIndex = (md, userSuppliedOptions) => {
  ...
  const defaultFencedCodeRenderer = md.renderer.rules.fence || defaultRenderToken;
  md.renderer.rules.fence = makeTabIndexRenderer(defaultFencedCodeRenderer, options);
};

module.exports = markdownItCodeTabIndex;
